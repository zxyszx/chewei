...
export async function GET(request: Request) {
  ...
  const sheet = workbook.addWorksheet("续费记录", {
  ...
  sheet.columns = [
    ...
    { header: "支付方式", key: "payment", width: 14 },
  ...
  for (const row of records)
    sheet.addRow({
      ...
      member: row.member.nickname,
      platform: row.slot.platform.name,
      slot: `#${row.slot.slotNumber}`,
      old: row.oldExpireDate,
      next: row.newExpireDate,
      amount: Number(row.amount),
      payment: payment[row.paymentMethod],
  ...
  return new Response(buffer as ArrayBuffer, {
    headers: {
      ...
      "content-disposition": `attachment; filename="renewals-${new Date().toISOString().slice(0, 10)}.xlsx"`,
    },
  });
}
