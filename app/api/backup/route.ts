...
export async function GET() {
  ...
  const [platforms, slots, members, renewals, operationLogs, users, settings] = await Promise.all([
    ...
    prisma.member.findMany(), prisma.renewal.findMany(), prisma.operationLog.findMany(), prisma.user.findMany({ select: { id: true, username: true, role: true, status: true, createdAt: true, updatedAt: true } }), prisma.setting.findMany(),
  ]);
  const body = JSON.stringify({ version: 2, exportedAt: new Date().toISOString(), platforms, slots, members, renewals: renewals.map((r) => ({ ...r, amount: r.amount.toString() })), operationLogs, users, settings }, null, 2);
