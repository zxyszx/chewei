...
import { resolveUpdateStatus, type UpdateStatus } from "@/lib/update-status";
...
async function readStatus() {
  ...
  try { return JSON.parse(await readFile(statusFile, "utf8")) as UpdateStatus; } catch { return null; }
}
...
export async function GET() {
  ...
  try {
    ...
    return Response.json({ enabled: process.env.WEB_UPDATE_ENABLED === "true" && Boolean(requestFile), current, latest, updateAvailable: current !== "unknown" && current !== latest.sha, status: resolveUpdateStatus(await readStatus(), current, latest.sha) });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  if (statusFile) {
    ...
    await writeFile(statusTemporary, JSON.stringify({ state: "queued", message: "已提交更新请求", updatedAt: new Date().toISOString() }), { mode: 0o600 });
    await rename(statusTemporary, statusFile);
  }
