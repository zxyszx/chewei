...
export default async function OverviewPage() {
  const now = new Date();
  const [slots, members, reminders] = await Promise.all([
    ...
    prisma.member.findMany({ where: { status: "ACTIVE", expireDate: { lte: addDays(now, 30) } }, include: { slot: { include: { platform: true } } }, orderBy: { expireDate: "asc" }, take: 8 }),
  ]);
  ...
  const upcoming = reminders.filter((m) => m.expireDate >= now).length;
  const expired = reminders.filter((m) => m.expireDate < now).length;
