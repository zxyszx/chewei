...
import { Activity, AlertTriangle, ArrowRight, BellRing, CalendarClock, CircleParking, Plus, ReceiptText, ShieldCheck, ShieldAlert, UsersRound } from "lucide-react";
...
export default async function OverviewPage() {
  ...
  }).filter((platform) => platform.capacity > 0).sort((a, b) => b.used - a.used || b.percentage - a.percentage);
  const expiringMembers = allMembers
    .map((member) => ({ ...member, days: dayDiff(member.expireDate, today) }))
    .filter((member) => member.days >= 0 && member.days <= 30)
  ...
  return <div className="mx-auto max-w-[1600px] space-y-5">
    ...
    <section aria-labelledby="pending-title"><h2 id="pending-title" className="section-heading mb-3">待处理事项</h2><div className="panel grid overflow-hidden sm:grid-cols-2 xl:grid-cols-4">{pendingItems.map(([label, value, href, Icon, tone], index) => <Link key={label} href={href} className={`group flex min-h-[76px] items-center gap-3 border-b border-[var(--border)] px-4 py-3 transition-colors hover:bg-[var(--surface-subtle)] sm:[&:nth-child(odd)]:border-r xl:border-b-0 xl:border-r ${index === pendingItems.length - 1 ? "xl:border-r-0" : ""}`}><span className={`metric-icon size-9 ${tone === "red" ? "icon-tone-red" : "icon-tone-orange"}`}><Icon size={17} /></span><span className="min-w-0 flex-1"><span className="block text-[13px] font-semibold">{label}</span><strong className={`mt-0.5 block text-[19px] tabular ${tone === "red" ? "text-[var(--danger)]" : "text-[var(--warning)]"}`}>{value}</strong></span><ArrowRight size={15} className="text-[var(--muted-foreground)] transition-transform group-hover:translate-x-0.5" /></Link>)}</div></section>

    <section className="grid items-start gap-4 xl:grid-cols-[3fr_2fr]">
      <div><div className="mb-3 flex items-center justify-between"><h2 className="section-heading">平台占用率</h2><Link href="/analytics" className="text-[13px] font-semibold text-[var(--accent)]">查看统计</Link></div><div className="panel divide-y divide-[var(--border)]">{platforms.slice(0, 5).map((platform) => <div key={platform.id} className="overview-platform-row grid grid-cols-[minmax(110px,0.7fr)_minmax(120px,1.8fr)_72px] items-center gap-4 px-4 py-3"><span className="flex min-w-0 items-center gap-3"><PlatformIcon slug={platform.slug} name={platform.name} icon={platform.icon} size={26} className="border border-[var(--border)]" /><strong className="truncate text-[13px]">{platform.name}</strong></span><ProgressBar value={platform.percentage} label={`${platform.name} 席位占用率 ${platform.percentage}%`} /><span className="text-right text-[12px] tabular"><strong>{platform.percentage}%</strong><span className="mt-0.5 block text-[var(--muted-foreground)]">{platform.used}/{platform.capacity}</span></span></div>)}{!platforms.length && <div className="empty">暂无有效平台席位</div>}</div></div>
      <div className="space-y-4"><div><div className="mb-3 flex items-center justify-between"><h2 className="section-heading">最近操作</h2><Link href="/logs" className="text-[13px] font-semibold text-[var(--accent)]">查看全部</Link></div><div className="panel divide-y divide-[var(--border)]">{recentOperations.slice(0, 3).map((log) => <div key={log.id} className="flex min-h-[56px] items-center gap-3 px-4 py-2.5"><span className="metric-icon size-8 icon-tone-blue"><Activity size={15} /></span><div className="min-w-0 flex-1"><p className="truncate text-[13px]"><strong>{log.user.username}</strong> · {operationNames[log.action] || log.action}</p></div><time className="shrink-0 text-[11px] tabular text-[var(--muted-foreground)]">{format(log.createdAt, "MM.dd HH:mm")}</time></div>)}{!recentOperations.length && <div className="empty">暂无操作记录</div>}</div></div>
      <div className="panel overflow-hidden"><div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3"><h2 className="text-[13px] font-semibold">需关注</h2><Link href="/reminders?range=30" className="text-[12px] font-semibold text-[var(--accent)]">查看提醒</Link></div>{expiringMembers.map((member) => <Link key={member.id} href={`/slots?open=${member.slot.id}`} className="flex items-center gap-3 border-b border-[var(--border)] px-4 py-2.5 last:border-b-0 hover:bg-[var(--surface-subtle)]"><CalendarClock size={15} className="text-[var(--warning)]" /><span className="min-w-0 flex-1 truncate text-[12px]">{member.nickname} · {member.slot.platform.name} #{member.slot.slotNumber}</span><Badge tone={member.days <= 7 ? "warning" : "blue"}>{member.days === 0 ? "今天" : `${member.days} 天`}</Badge></Link>)}{!expiringMembers.length && <div className="flex items-center gap-2 px-4 py-3 text-[12px] text-[var(--muted-foreground)]"><ShieldCheck size={16} className="text-[var(--success)]" />30 天内暂无到期车友</div>}<Link href="/slots?status=attention" className="flex items-center gap-3 border-t border-[var(--border)] px-4 py-2.5 hover:bg-[var(--surface-subtle)]">{abnormal > 0 ? <ShieldAlert size={15} className="text-[var(--danger)]" /> : <ShieldCheck size={15} className="text-[var(--success)]" />}<span className="flex-1 text-[12px]">异常或停用账号</span><Badge tone={abnormal > 0 ? "danger" : "neutral"}>{abnormal}</Badge></Link></div></div>
    </section>
