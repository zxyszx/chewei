        ...
        <div className="flex gap-1 overflow-x-auto border-b border-[var(--border)] px-3 py-2">
          {tabs.map(([value, label]) => (
            <Link
              ...
              className={cn(
                "flex min-h-9 items-center gap-2 whitespace-nowrap rounded-[5px] px-3 py-2 text-[12px] font-medium text-[#657080] hover:bg-[#f3f5f7]",
                range === value && "bg-[#edf3ff] text-[#2457bd]",
              )}
            >
              {label}
              <span className="rounded-full bg-white px-1.5 py-0.5 text-[10px] tabular text-[#667085]">
                {tabCount(value)}
              </span>
            </Link>
          ))}
