...
export default async function RemindersPage({ searchParams }: { searchParams: Promise<{ range?: string }> }) {
  ...
  return <div className="mx-auto max-w-[1700px] space-y-4"><PageHeader title="到期提醒" description="到期状态根据车友日期自动计算" />
    <section className="panel overflow-hidden"><div className="flex gap-1 overflow-x-auto border-b border-[var(--border)] px-3 py-2">{tabs.map(([value, label]) => <Link key={value} href={`/reminders?range=${value}`} className={cn("min-h-9 whitespace-nowrap rounded-[5px] px-3 py-2 text-[12px] font-medium text-[#657080] hover:bg-[#f3f5f7]", range === value && "bg-[#edf3ff] text-[#2457bd]")}>{label}</Link>)}</div><div className="data-wrap"><table className="data-table"><thead><tr><th>车友</th><th>平台</th><th>车位</th><th>联系方式</th><th>到期时间</th><th>剩余时间</th><th>状态</th><th>操作</th></tr></thead><tbody>{members.map((member) => { const expiry = expiryLabel(member.expireDate); const item: MemberItem = { id: member.id, nickname: member.nickname, contact: member.contact, startDate: member.startDate.toISOString(), expireDate: member.expireDate.toISOString(), status: member.status, note: member.note }; return <tr key={member.id}><td className="font-semibold">{member.nickname}</td><td>{member.slot.platform.name}</td><td><Link className="text-[#2457bd]" href={`/slots?open=${member.slotId}`}>#{member.slot.slotNumber}</Link></td><td>{member.contact}</td><td className="tabular">{format(member.expireDate, "yyyy.MM.dd")}</td><td><Badge tone={expiry.tone}>{expiry.text}</Badge></td><td>{expiry.days < 0 ? <Badge tone="danger">已过期</Badge> : expiry.days <= 7 ? <Badge tone="warning">需关注</Badge> : <Badge tone="success">正常</Badge>}</td><td><RenewButton member={item} compact /></td></tr>; })}</tbody></table>{!members.length && <div className="empty">当前筛选范围内没有到期记录</div>}</div></section>
  </div>;
}
