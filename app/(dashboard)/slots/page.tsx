...
export default async function SlotsPage({ searchParams }: { searchParams: Promise<{ platform?: string; open?: string }> }) {
  ...
  return <div className="mx-auto max-w-[1800px] space-y-4"><PageHeader title={params.platform ? `${rows[0]?.platform.name || "平台"} · 车位管理` : "车位管理"} description="统一管理账号容量、车友与到期信息" /><SlotManager slots={slots} platforms={platforms.map((p) => ({ id: p.id, name: p.name, slug: p.slug, defaultCapacity: p.defaultCapacity }))} initialOpen={params.open} /></div>;
}
