...
export default async function MembersPage({ searchParams }: { searchParams: Promise<{ q?: string; platform?: string; status?: string; expiry?: string; page?: string; size?: string }> }) {
  ...
  return <div className="mx-auto max-w-[1800px] space-y-4">
    ...
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-4"><MetricCard label="在位车友" value={active} icon={UserRoundCheck} tone="green" detail="当前正常订阅" /><MetricCard label="30 天内到期" value={upcoming} icon={Clock3} tone="orange" detail="需要提前跟进" /><MetricCard label="已过期" value={expired} icon={AlertTriangle} tone="red" detail="仍在位且已到期" /><MetricCard label="本月新增" value={addedThisMonth} icon={UserRoundPlus} tone="blue" detail="本月加入车友" /></section>
