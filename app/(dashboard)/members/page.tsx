...
export default async function MembersPage({ searchParams }: { searchParams: Promise<{ q?: string; platform?: string; status?: string; expiry?: string; page?: string }> }) {
  const { q = "", platform = "", status = "", expiry = "", page: pageParam = "1" } = await searchParams;
  ...
  const [platforms, members, matched, total, active, upcoming, expired, addedToday] = await Promise.all([
    ...
    prisma.member.count({ where: { createdAt: { gte: today, lt: addDays(today, 1) } } }),
  ]);
  ...
  return <div className="mx-auto max-w-[1800px] space-y-4">
    ...
    <section className="panel overflow-hidden">
      ...
      <Pagination page={page} total={matched} pageSize={pageSize} pathname="/members" params={{ q, platform, status, expiry }} />
    </section>
  </div>;
}
