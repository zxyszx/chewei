...
进入“系统设置 > 备份与恢复”可创建服务器本地 JSON 完整备份。页面会列出最近 10 份备份，并提供刷新、校验、下载和删除操作；备份文件持久保存在 `/opt/chewei/data/backups/`，容器重建或应用更新不会清除。

也可以上传 JSON 备份恢复全部数据。备份包含业务数据、管理员密码哈希以及加密后的平台密码和验证码链接，但昵称、联系方式等业务字段仍为可读数据，因此下载后的文件仍应按敏感资料妥善保管。

恢复到新服务器时必须同时保留原服务器 `.env.production` 中的 `ENCRYPTION_KEY`，否则旧平台密码和验证码链接无法解密。恢复会覆盖当前数据并退出当前登录，请先下载当前备份。
