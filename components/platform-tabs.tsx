...
export function PlatformTabs({ platforms, current }: { platforms: PlatformTab[]; current?: string }) {
  ...
  const shown = hiddenCurrent ? [...sorted.slice(0, visibleCount - 1), sorted.find((item) => item.slug === current)!] : sorted.slice(0, visibleCount);
  const hidden = sorted.filter((item) => !shown.some((shownItem) => shownItem.id === item.id));
  ...
  return <nav className="platform-tabs" aria-label="合租车位工作表">
    ...
    {hidden.length > 0 && <details name="chewei-popover" className="platform-more-menu relative"><summary className="platform-tab platform-tab-more">更多 <span>{hidden.length}</span><ChevronDown size={14} /></summary><div className="platform-more-popover">{hidden.map((platform) => <Link key={platform.id} href={`/slots?platform=${encodeURIComponent(platform.slug)}`} scroll={false}><PlatformIcon slug={platform.slug} name={platform.name} icon={platform.icon} size={16} /><span>{platform.name}</span><small>{platform.count}</small></Link>)}</div></details>}
