...
import { createPlatformAction, deletePlatformAction, updatePlatformAction, updateRemindersAction } from "@/app/actions";
...
function NewPlatformForm({ close }: { close: () => void }) {
  return <ActionForm action={createPlatformAction} onSuccess={close} className="grid gap-4 sm:grid-cols-2">
    ...
    <div><label className="label" htmlFor="newPlatformCapacity">默认容量</label><input className="input" id="newPlatformCapacity" name="defaultCapacity" type="number" min="1" max="99" defaultValue="5" required /></div>
...
export function PlatformSettings({ platforms, editable }: { platforms: PlatformRecord[]; editable: boolean }) {
  ...
  const [deleting, setDeleting] = useState<PlatformRecord | null>(null);
  const [pending, startTransition] = useTransition();
  const remove = () => startTransition(async () => {
    if (!deleting) return;
    const result = await deletePlatformAction(deleting.id);
    if (result.ok) { toast.success(result.message); setDeleting(null); router.refresh(); }
    else toast.error(result.message);
  });

  return <>
    <div className="flex items-center justify-between border-b border-[var(--border)] px-5 py-3.5"><div><h2 className="font-semibold">平台设置</h2><p className="mt-0.5 text-[11px] text-[var(--muted-foreground)]">名称、图标、默认容量与可用状态</p></div>{editable && <button className="btn btn-primary min-h-9 px-3 text-[12px]" onClick={() => setNewPlatform(true)}><Plus size={15} />新增平台</button>}</div>
    <div className="data-wrap"><table className="data-table"><thead><tr><th>图标</th><th>平台名称</th><th>标识</th><th>默认容量</th><th>状态</th><th>车位数</th><th>图标文件</th><th>操作</th></tr></thead><tbody>{platforms.map((platform) => <tr key={platform.id}><td><PlatformIcon slug={platform.slug} name={platform.name} icon={platform.icon} size={20} className="border border-[var(--border)]" /></td><td colSpan={7} className="!p-0"><ActionForm action={updatePlatformAction} className="grid min-w-[900px] grid-cols-[minmax(150px,1fr)_minmax(140px,1fr)_100px_120px_80px_minmax(190px,1fr)_150px] items-center gap-3 px-3 py-2"><input type="hidden" name="platformId" value={platform.id} /><input className="input min-h-9" name="name" defaultValue={platform.name} disabled={!editable} aria-label={`${platform.name} 名称`} /><span className="truncate font-mono text-[12px] text-[var(--muted-foreground)]">{platform.slug}</span><input className="input min-h-9" name="defaultCapacity" type="number" min="1" max="99" defaultValue={platform.defaultCapacity} disabled={!editable} aria-label={`${platform.name} 默认容量`} /><select className="select min-h-9" name="status" defaultValue={platform.status === "ACTIVE" ? "ACTIVE" : "PAUSED"} disabled={!editable} aria-label={`${platform.name} 状态`}><option value="ACTIVE">启用</option><option value="PAUSED">停用</option></select><span className="tabular text-[12px]">{platform.slotCount}</span><label className="btn min-h-9 cursor-pointer px-2 text-[12px]"><ImagePlus size={14} />选择图片<input className="sr-only" name="iconFile" type="file" accept="image/png,image/jpeg,image/webp" disabled={!editable} /></label><div className="flex gap-1">{editable && <SubmitButton className="min-h-9 px-2.5 text-[12px]">更新</SubmitButton>}{editable && platform.icon && <button className="btn icon-btn size-9" type="submit" name="intent" value="reset-icon" aria-label={`恢复 ${platform.name} 默认图标`} title="恢复默认图标"><RotateCcw size={14} /></button>}{editable && <button className="btn btn-danger icon-btn size-9" type="button" onClick={() => setDeleting(platform)} aria-label={`删除 ${platform.name}`} title="删除平台"><Trash2 size={14} /></button>}</div></ActionForm></td></tr>)}</tbody></table></div>
    <FormDialog open={newPlatform} title="新增平台" description="添加后即可创建该平台的共享车位" onClose={() => setNewPlatform(false)}><NewPlatformForm close={() => setNewPlatform(false)} /></FormDialog>
    {deleting && <ConfirmDialog open title={`删除 ${deleting.name}？`} description={deleting.slotCount ? `该平台已有 ${deleting.slotCount} 个车位，不能删除。可以将平台状态改为停用。` : "删除后该平台将从导航和设置中移除，此操作不可恢复。"} confirmLabel="删除平台" pending={pending} onClose={() => setDeleting(null)} onConfirm={remove} />}
  </>;
}
