...
function NewPlatformForm({ close, existingSlugs }: { close: () => void; existingSlugs: string[] }) {
  ...
  const results = searchPlatformCatalog(query).filter((item) => availableCatalog.some((available) => available.slug === item.slug)).slice(0, 12);
  ...
  return <ActionForm action={createPlatformAction} onSuccess={close} className="grid gap-4 sm:grid-cols-2">
    <div className="sm:col-span-2"><span className="label">常用平台</span><div className="platform-catalog-picker"><div className="relative"><Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--muted-foreground)]" /><input className="input pl-9" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索 Netflix、ChatGPT、爱奇艺..." aria-label="搜索常用平台" /></div><div className="platform-catalog-grid">{results.map((item) => <button key={item.slug} type="button" className={`platform-catalog-item ${!custom && selected?.slug === item.slug ? "platform-catalog-item-active" : ""}`} onClick={() => choose(item)}><PlatformIcon slug={item.slug} name={item.name} size={20} /><span className="min-w-0 flex-1 text-left"><strong className="block truncate text-[12px]">{item.name}</strong><small>{item.category} · {item.defaultCapacity} 席</small></span>{!custom && selected?.slug === item.slug && <Check size={15} />}</button>)}{results.length === 0 && <p className="col-span-full py-5 text-center text-[12px] text-[var(--muted-foreground)]">没有匹配项，可切换到自定义平台</p>}</div><button type="button" className="mt-2 text-[12px] font-semibold text-[var(--accent)]" onClick={createCustom}>找不到？创建自定义平台</button></div></div>
    <div><label className="label" htmlFor="newPlatformName">平台名称</label><input className="input" id="newPlatformName" name="name" required maxLength={60} value={name} readOnly={!custom} onChange={(event) => setName(event.target.value)} placeholder="例如 Netflix" /></div>
    <div><label className="label" htmlFor="newPlatformSlug">平台标识</label><input className="input" id="newPlatformSlug" name="slug" required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" value={slug} readOnly={!custom} onChange={(event) => setSlug(event.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "-"))} placeholder="例如 youtube-premium" /></div>
    <div><label className="label" htmlFor="newPlatformCapacity">默认席位数</label><input className="input" id="newPlatformCapacity" name="defaultCapacity" type="number" min="1" max="99" value={capacity} onChange={(event) => setCapacity(Number(event.target.value))} required /></div>
