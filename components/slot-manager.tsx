...
function SeatButtons({ slot, members, onMember, onEmpty }: { slot: SlotItem; members: MemberItem[]; onMember: (member: MemberItem) => void; onEmpty: (seatNumber: number) => void }) {
  return <div className="flex min-w-max items-center gap-1.5" aria-label={`${slot.platform.name} 共享账号 #${slot.slotNumber}，已使用 ${members.length}/${slot.capacity} 个席位`}>
...
function NewSlotForm({ platforms, close }: { platforms: PlatformOption[]; close: () => void }) {
  ...
  return <ActionForm action={createSlotAction} onSuccess={close} className="grid gap-4 sm:grid-cols-2">
    ...
    <div className="flex justify-end gap-2 border-t border-[var(--border)] pt-4 sm:col-span-2"><button type="button" className="btn" onClick={close}>取消</button><SubmitButton>保存账号</SubmitButton></div>
  </ActionForm>;
}
...
function MoveMemberForm({ member, currentSlot, slots, close }: { member: MemberItem; currentSlot: SlotItem; slots: SlotItem[]; close: () => void }) {
  ...
  return <ActionForm action={moveMemberFormAction} onSuccess={close} className="space-y-4"><input type="hidden" name="memberId" value={member.id} /><div><label className="label" htmlFor="targetSlotId">目标共享账号</label><select className="select" id="targetSlotId" name="targetSlotId" required defaultValue=""><option value="" disabled>请选择有空余席位的账号</option>{available.map((slot) => { const count = slot.members.filter((item) => item.status === "ACTIVE").length; return <option key={slot.id} value={slot.id}>{slot.platform.name} 账号 #{slot.slotNumber} · {count}/{slot.capacity}</option>; })}</select></div>{!available.length && <p className="rounded-[6px] bg-[#fff5e8] p-3 text-[12px] text-[#9a4e08]">当前没有可用的目标共享账号</p>}<div className="flex justify-end gap-2 border-t border-[var(--border)] pt-4"><button type="button" className="btn" onClick={close}>取消</button><SubmitButton disabled={!available.length}>确认更换</SubmitButton></div></ActionForm>;
}
...
function SlotDrawer({ slot, focusedMemberId, close, addMember, editSlot, editMember, renew, move, canDelete }: { slot: SlotItem; focusedMemberId?: string; close: () => void; addMember: () => void; editSlot: () => void; editMember: (member: MemberItem) => void; renew: (member: MemberItem) => void; move: (member: MemberItem) => void; canDelete: boolean }) {
  ...
  const confirmationCopy = confirmation?.kind === "delete-slot"
    ? { title: `删除 ${slot.platform.name} 账号 #${slot.slotNumber}`, description: "只有没有成员和续费历史的共享账号才能删除。删除后无法恢复。", confirmLabel: "永久删除", tone: "danger" as const }
    : confirmation?.kind === "delete-member"
      ...
      : confirmation?.kind === "exit"
        ...
        : null;
  return <><div className="fixed inset-0 z-[70] bg-black/30" onMouseDown={close}><aside ref={drawer} role="dialog" aria-modal="true" className="absolute inset-y-0 right-0 flex w-full max-w-[500px] flex-col border-l border-[var(--border)] bg-[var(--surface)] shadow-2xl" onMouseDown={(e) => e.stopPropagation()} aria-label={`共享账号 ${slot.slotNumber} 详情`}>
    <div className="flex min-h-[58px] items-center justify-between gap-3 border-b border-[var(--border)] px-5"><div className="min-w-0"><strong className="text-[15px]">共享账号 #{slot.slotNumber}</strong><span className="ml-3 text-[11px] text-[var(--muted-foreground)]">ID: {publicId(slot.platform.slug, slot.slotNumber)}</span></div><div className="flex items-center gap-1"><details className="action-menu relative"><summary className="btn icon-btn" aria-label="更多账号操作" title="更多操作"><MoreHorizontal size={17} /></summary><div className="menu-popover absolute right-0 top-[calc(100%+6px)] z-20 w-36 rounded-[7px] border border-[var(--border)] p-1.5 shadow-lg"><button className="menu-item w-full" onClick={editSlot}><Pencil size={14} />编辑账号</button>{canDelete && <button className="menu-item menu-item-danger w-full" disabled={pending} onClick={() => setConfirmation({ kind: "delete-slot" })}><Trash2 size={14} />删除账号</button>}</div></details><button ref={closeButton} className="btn icon-btn" onClick={close} aria-label="关闭详情" title="关闭详情"><X size={17} /></button></div></div>
...
export function SlotManager({ slots, platforms, initialOpen, initialCreate = false, closeHref = "/slots", singlePlatform = false, platformSlug, canDelete = false }: { slots: SlotItem[]; platforms: PlatformOption[]; initialOpen?: string; initialCreate?: boolean; closeHref?: string; singlePlatform?: boolean; platformSlug?: string; canDelete?: boolean }) {
  ...
  return <>
    <div className="panel overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] px-4 py-3"><div className="flex items-center gap-3"><strong className="text-[15px]">账号列表 <span className="font-normal text-[var(--muted-foreground)]">({filtered.length})</span></strong><div className="flex rounded-[6px] border border-[var(--border)] bg-[#fafbfc] p-0.5">{viewOptions.map(({ value, icon: Icon, label }) => <button key={value} onClick={() => setView(value)} className={cn("flex min-h-8 items-center gap-1.5 rounded-[4px] px-2.5 text-[12px] text-[#657080]", view === value && "bg-white text-[#2457bd] shadow-sm")}><Icon size={14} />{label}</button>)}</div></div><button className="btn btn-primary" onClick={() => setNewSlot(true)}><Plus size={16} />新增账号</button></div>
    ...
    <FormDialog open={newSlot} title="新增共享账号" description="录入平台登录信息，并按套餐设置成员席位" onClose={closeNewSlot}><NewSlotForm platforms={platforms} close={closeNewSlot} /></FormDialog>
    {selected && <FormDialog open={editSlot} title={`编辑共享账号 · ${selected.platform.name} #${selected.slotNumber}`} description="留空密码字段即可保留原密码" onClose={() => setEditSlot(false)}><EditSlotForm slot={selected} platforms={platforms} close={() => setEditSlot(false)} /></FormDialog>}
    ...
    {moveMember && selected && <FormDialog open title={`更换账号席位 · ${moveMember.nickname}`} description={`仅显示 ${selected.platform.name} 有空余席位的共享账号，续费历史保持不变`} onClose={() => setMoveMember(null)}><MoveMemberForm member={moveMember} currentSlot={selected} slots={slots} close={() => setMoveMember(null)} /></FormDialog>}
  </>;
}
