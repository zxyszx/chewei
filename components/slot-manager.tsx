...
function SlotDrawer({ slot, focusedMemberId, close, addMember, editSlot, editMember, renew, move, canDelete }: { slot: SlotItem; focusedMemberId?: string; close: () => void; addMember: () => void; editSlot: () => void; editMember: (member: MemberItem) => void; renew: (member: MemberItem) => void; move: (member: MemberItem) => void; canDelete: boolean }) {
  ...
  const confirmationCopy = confirmation?.kind === "delete-slot"
    ...
    : confirmation?.kind === "delete-member"
      ...
      : confirmation?.kind === "exit"
        ...
        : null;
  const tabs = [
    { id: "members" as const, label: "成员席位", count: `${activeMembers.length}/${slot.capacity}` },
    { id: "renewals" as const, label: "续费记录", count: String(slot.renewals.length) },
  ...
  return <><div className="fixed inset-0 z-[70] bg-black/30" onMouseDown={close}><aside ref={drawer} role="dialog" aria-modal="true" className="absolute inset-y-0 right-0 flex w-full max-w-[500px] flex-col border-l border-[var(--border)] bg-[var(--surface)] shadow-2xl" onMouseDown={(e) => e.stopPropagation()} aria-label={`合租车位 ${slot.slotNumber} 详情`}>
    ...
    <div className="drawer-tabs" role="tablist" aria-label="账号详情分类">{tabs.map((tab) => <button key={tab.id} type="button" role="tab" aria-selected={activeTab === tab.id} className={cn("drawer-tab", activeTab === tab.id && "drawer-tab-active")} onClick={() => setActiveTab(tab.id)}>{tab.label}{tab.count && <span>{tab.count}</span>}</button>)}</div>
    <div className="min-h-0 flex-1 overflow-y-auto">
      ...
      {activeTab === "members" && <section role="tabpanel" className="px-5 py-5"><div className="mb-3 flex items-center justify-between"><div><h3 className="font-semibold">在位车友</h3><p className="mt-0.5 text-[11px] text-[var(--muted-foreground)]">点击车友资料直接编辑</p></div><button className="btn min-h-8 px-2.5 text-[12px] text-[#2563eb]" onClick={addMember} disabled={activeMembers.length >= slot.capacity}><UserPlus size={14} />添加车友</button></div>
        <div className="overflow-visible rounded-[6px] border border-[var(--border)]">{activeMembers.length ? activeMembers.map((member) => <div key={member.id} className={cn("grid gap-3 border-b border-[var(--border)] p-3 last:border-0 sm:grid-cols-[minmax(0,1fr)_auto]", focusedMemberId === member.id && "bg-[var(--accent-soft)]")}><button type="button" className="min-w-0 text-left" onClick={() => editMember(member)} aria-label={`编辑车友 ${member.nickname}`}><div className="flex items-center gap-2"><span className="grid size-7 shrink-0 place-items-center rounded-full bg-[var(--accent-soft)] text-[11px] font-semibold text-[var(--accent)]">{member.nickname.slice(0, 1).toUpperCase()}</span><strong className="truncate text-[13px]">{member.nickname}</strong><Expiry value={member.expireDate} /></div><div className="mt-1 flex min-w-0 items-center gap-1.5 pl-9 text-[12px] text-[var(--muted-foreground)]"><ContactValue type={member.contactType} value={member.contact} className="max-w-[165px]" /><span>·</span><span className="truncate">{format(new Date(member.startDate), "yyyy.MM.dd")} 至 {format(new Date(member.expireDate), "yyyy.MM.dd")}</span></div>{member.note && <p className="mt-1 truncate pl-9 text-[11px] text-[var(--muted-foreground)]">{member.note}</p>}</button><div className="flex items-center gap-1"><button className="btn btn-primary min-h-8 px-2.5 text-[12px]" onClick={() => renew(member)}>续费</button><details className="action-menu relative"><summary className="btn icon-btn size-8" aria-label={`${member.nickname} 更多操作`} title="更多操作"><MoreHorizontal size={15} /></summary><div className="menu-popover absolute right-0 top-[calc(100%+5px)] z-20 w-36 rounded-[7px] border border-[var(--border)] p-1.5 shadow-lg"><button className="menu-item w-full" onClick={() => { closeOpenActionMenus(); move(member); }}>换位</button><button className="menu-item w-full" onClick={() => { closeOpenActionMenus(); copy(member.contact, "联系方式已复制"); }}><Copy size={14} />复制联系</button><button className="menu-item w-full" onClick={() => { closeOpenActionMenus(); editMember(member); }}><Pencil size={14} />编辑车友</button><button className="menu-item w-full" disabled={pending} onClick={() => { closeOpenActionMenus(); setConfirmation({ kind: "exit", member }); }}><UserRoundX size={14} />标记退出</button>{canDelete && <button className="menu-item menu-item-danger w-full" disabled={pending} onClick={() => { closeOpenActionMenus(); setConfirmation({ kind: "delete-member", member }); }}><Trash2 size={14} />永久删除</button>}</div></details></div></div>) : <div className="empty py-8">暂无在位车友</div>}</div>
      </section>}
      {activeTab === "renewals" && <section role="tabpanel" className="px-5 py-5"><div className="mb-3 flex items-center justify-between"><div><h3 className="font-semibold">续费记录</h3><p className="mt-0.5 text-[11px] text-[var(--muted-foreground)]">按操作时间从近到远排列</p></div><span className="text-[12px] text-[var(--muted-foreground)]">共 {slot.renewals.length} 条</span></div><div className="overflow-hidden rounded-[6px] border border-[var(--border)]">{slot.renewals.length ? slot.renewals.map((record) => <div key={record.id} className="grid grid-cols-[1fr_auto] border-b border-[var(--border)] px-3 py-3 text-[12px] last:border-0"><div><strong>{record.member.nickname}</strong><p className="mt-0.5 text-[11px] text-[var(--muted-foreground)]">{format(new Date(record.createdAt), "yyyy.MM.dd")} · {paymentMethodLabels[record.paymentMethod] || record.paymentMethod}</p></div><span className="tabular">¥ {Number(record.amount).toFixed(2)}</span></div>) : <div className="empty py-10">暂无续费记录</div>}</div></section>}
    </div>
...
export function SlotManager({ slots, platforms, initialOpen, initialCreate = false, closeHref = "/slots", singlePlatform = false, canDelete = false }: { slots: SlotItem[]; platforms: PlatformOption[]; initialOpen?: string; initialCreate?: boolean; closeHref?: string; singlePlatform?: boolean; platformSlug?: string; canDelete?: boolean }) {
