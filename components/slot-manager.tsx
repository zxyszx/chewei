...
import { CalendarDays, ChevronLeft, ChevronRight, Clipboard, Columns3, Copy, Eye, EyeOff, Pencil, Plus, Search, Table2, Trash2, UserPlus, UserRoundX, X } from "lucide-react";
...
function SeatButtons({ slot, members, onMember, onEmpty }: { slot: SlotItem; members: MemberItem[]; onMember: (member: MemberItem) => void; onEmpty: (seatNumber: number) => void }) {
  return <div className="flex min-w-max items-center gap-1.5" aria-label={`${slot.platform.name} 车位 #${slot.slotNumber}，已使用 ${members.length}/${slot.capacity}`}>
...
function SlotDrawer({ slot, focusedMemberId, close, addMember, editSlot, editMember, renew, move, canDelete }: { slot: SlotItem; focusedMemberId?: string; close: () => void; addMember: () => void; editSlot: () => void; editMember: (member: MemberItem) => void; renew: (member: MemberItem) => void; move: (member: MemberItem) => void; canDelete: boolean }) {
  ...
  const confirmationCopy = confirmation?.kind === "delete-slot"
    ? { title: `删除 ${slot.platform.name} #${slot.slotNumber}`, description: "只有没有车友和续费历史的空车位才能删除。删除后无法恢复。", confirmLabel: "永久删除", tone: "danger" as const }
  ...
  return <><div className="fixed inset-0 z-[70] bg-black/30" onMouseDown={close}><aside ref={drawer} role="dialog" aria-modal="true" className="absolute inset-y-0 right-0 flex w-full max-w-[500px] flex-col border-l border-[var(--border)] bg-white shadow-2xl" onMouseDown={(e) => e.stopPropagation()} aria-label={`车位 ${slot.slotNumber} 详情`}>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto">
      ...
      <section className="border-b border-[var(--border)] px-5 py-5"><div className="mb-3 flex items-center justify-between"><h3 className="font-semibold">车友列表 <span className="font-normal text-[var(--muted-foreground)]">({activeMembers.length}/{slot.capacity})</span></h3><button className="btn min-h-8 px-2.5 text-[12px] text-[#2563eb]" onClick={addMember} disabled={activeMembers.length >= slot.capacity}><UserPlus size={14} />添加车友</button></div>
        <div className="overflow-hidden rounded-[6px] border border-[var(--border)]">{activeMembers.length ? activeMembers.map((member) => <div key={member.id} className={cn("grid gap-3 border-b border-[var(--border)] p-3 last:border-0 sm:grid-cols-[minmax(0,1fr)_auto]", focusedMemberId === member.id && "bg-[#f2f6ff]")}><div className="min-w-0"><div className="flex items-center gap-2"><span className="grid size-7 shrink-0 place-items-center rounded-full bg-[#edf3ff] text-[11px] font-semibold text-[#2457bd]">{member.nickname.slice(0, 1).toUpperCase()}</span><strong className="truncate text-[13px]">{member.nickname}</strong><Expiry value={member.expireDate} /></div><p className="mt-1 truncate pl-9 text-[12px] text-[var(--muted-foreground)]">{member.contact} · {format(new Date(member.startDate), "yyyy.MM.dd")} 至 {format(new Date(member.expireDate), "yyyy.MM.dd")}</p>{member.note && <p className="mt-1 truncate pl-9 text-[11px] text-[#8a94a3]">{member.note}</p>}</div><div className="flex flex-wrap items-center gap-1"><button className="btn min-h-8 px-2 text-[12px]" onClick={() => renew(member)}>续费</button><button className="btn min-h-8 px-2 text-[12px]" onClick={() => move(member)}>换位</button><button className="btn icon-btn size-8" onClick={() => copy(member.contact, "联系方式已复制")} aria-label={`复制 ${member.nickname} 的联系方式`} title="复制联系方式"><Copy size={14} /></button><button className="btn icon-btn size-8" onClick={() => editMember(member)} aria-label={`编辑 ${member.nickname}`} title="编辑车友"><Pencil size={14} /></button><button className="btn icon-btn size-8" disabled={pending} onClick={() => setConfirmation({ kind: "exit", member })} aria-label={`将 ${member.nickname} 标记退出`} title="标记退出"><UserRoundX size={15} /></button>{canDelete && <button className="btn btn-danger icon-btn size-8" disabled={pending} onClick={() => setConfirmation({ kind: "delete-member", member })} aria-label={`删除 ${member.nickname}`} title="删除车友"><Trash2 size={14} /></button>}</div></div>) : <div className="empty py-8">暂无在位车友</div>}</div>
      </section>
...
export function SlotManager({ slots, platforms, initialOpen, initialCreate = false, closeHref = "/slots", singlePlatform = false, platformSlug, canDelete = false }: { slots: SlotItem[]; platforms: PlatformOption[]; initialOpen?: string; initialCreate?: boolean; closeHref?: string; singlePlatform?: boolean; platformSlug?: string; canDelete?: boolean }) {
  ...
  return <>
    ...
    <FormDialog open={newSlot} title="新建车位" description="平台默认容量可按实际套餐调整" onClose={closeNewSlot}><NewSlotForm platforms={platforms} close={closeNewSlot} /></FormDialog>
    ...
    {moveMember && selected && <FormDialog open title={`更换车位 · ${moveMember.nickname}`} description={`仅显示 ${selected.platform.name} 的可用车位，续费历史保持不变`} onClose={() => setMoveMember(null)}><MoveMemberForm member={moveMember} currentSlot={selected} slots={slots} close={() => setMoveMember(null)} /></FormDialog>}
  </>;
}
