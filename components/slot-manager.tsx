...
export type SlotItem = { id: string; slotNumber: number; accountEmail: string; hasVerificationUrl: boolean; cardLast4: string | null; billingDay: number; capacity: number; status: string; note: string | null; platform: PlatformOption; members: MemberItem[]; renewals: RenewalItem[]; renewalCount: number };
...
function AccountCopyButton({ account, className }: { account: string; className?: string }) {
  return <button type="button" className={cn("group inline-flex max-w-full items-center gap-1.5 rounded-[5px] px-1 py-1 text-left text-[var(--accent)] hover:bg-[var(--accent-soft)]", className)} onClick={(event) => { event.stopPropagation(); void copyText(account, "登录账号已复制"); }} aria-label={`复制登录账号 ${account}`} title="点击复制登录账号"><span className="truncate">{account}</span><Copy size={13} className="shrink-0 opacity-0 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100" /></button>;
}
...
function GeneratedPasswordField({ id, label, required = false, value, onChange }: { id: string; label: string; required?: boolean; value: string; onChange: (value: string) => void }) {
  ...
  return <div className="sm:col-span-2">
    ...
    <div className="flex items-stretch overflow-hidden rounded-[6px] border border-[var(--border-strong)] bg-[var(--surface)] focus-within:border-[var(--accent)] focus-within:shadow-[0_0_0_3px_rgb(80_85_90/12%)]">
...
function ContactField({ id, defaultValue = "", defaultType }: { id: string; defaultValue?: string; defaultType?: string | null }) {
  ...
  return <div>
    ...
    <div className="flex min-w-0 overflow-hidden rounded-[6px] border border-[var(--border-strong)] bg-[var(--surface)] focus-within:border-[var(--accent)] focus-within:shadow-[0_0_0_3px_rgb(80_85_90/12%)]">
...
function NewSlotForm({ platforms, close }: { platforms: PlatformOption[]; close: () => void }) {
  ...
  return <ActionForm action={createSlotAction} onSuccess={close} className="grid gap-4 sm:grid-cols-2">
    ...
    <div className="sm:col-span-2"><label className="label" htmlFor="verificationUrl">验证码链接</label><input className="input" id="verificationUrl" name="verificationUrl" type="url" inputMode="url" autoCapitalize="off" autoCorrect="off" maxLength={2000} placeholder="https://...（可选，用于查看邮件验证码）" /><p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">链接会加密保存，列表中只能点击图标复制，不会直接显示。</p></div>
...
function EditSlotForm({ slot, platforms, close }: { slot: SlotItem; platforms: PlatformOption[]; close: () => void }) {
  const [password, setPassword] = useState("");
  const [clearVerificationUrl, setClearVerificationUrl] = useState(false);
  const complete = () => {
  ...
  return <ActionForm action={updateSlotAction} onSuccess={complete} className="grid gap-4 sm:grid-cols-2">
    ...
    <div className="sm:col-span-2"><label className="label" htmlFor="editVerificationUrl">验证码链接</label><input className="input" id="editVerificationUrl" name="verificationUrl" type="url" inputMode="url" autoCapitalize="off" autoCorrect="off" maxLength={2000} disabled={clearVerificationUrl} placeholder={clearVerificationUrl ? "保存后将移除现有链接" : slot.hasVerificationUrl ? "已设置；留空则保持原链接" : "https://...（可选）"} /><div className="mt-1.5 flex flex-wrap items-center justify-between gap-2 text-[11px] text-[var(--muted-foreground)]"><span>{clearVerificationUrl ? "已选择移除，取消勾选可重新填写。" : slot.hasVerificationUrl ? "当前链接已加密保存，可填写新链接覆盖。" : "保存后可从账号列表一键复制。"}</span>{slot.hasVerificationUrl && <label className="inline-flex cursor-pointer items-center gap-1.5"><input type="checkbox" name="clearVerificationUrl" value="true" checked={clearVerificationUrl} onChange={(event) => setClearVerificationUrl(event.target.checked)} />移除已有链接</label>}</div></div>
...
export function SlotManager({ slots, platforms, initialOpen, initialCreate = false, initialStatus = "all", closeHref = "/slots", singlePlatform = false, canDelete = false }: { slots: SlotItem[]; platforms: PlatformOption[]; initialOpen?: string; initialCreate?: boolean; initialStatus?: string; closeHref?: string; singlePlatform?: boolean; platformSlug?: string; canDelete?: boolean }) {
  ...
  const filtered = useMemo(() => {
    ...
    const result = slots.filter((slot) => {
      ...
      return statusMatch && (!normalizedQuery || `${slot.slotNumber} ${slot.accountEmail} ${slot.platform.name} ${slot.members.map((m) => `${m.nickname} ${m.contact}`).join(" ")}`.toLowerCase().includes(normalizedQuery));
    });
    const activeCount = (slot: SlotItem) => slot.members.filter((member) => member.status === "ACTIVE").length;
    const nextExpiry = (slot: SlotItem) => Math.min(...slot.members.filter((member) => member.status === "ACTIVE").map((member) => +new Date(member.expireDate)), Number.POSITIVE_INFINITY);
    return result.toSorted((a, b) => {
      if (sortMode === "expiry") return nextExpiry(a) - nextExpiry(b);
      if (sortMode === "availability") return (b.capacity - activeCount(b)) - (a.capacity - activeCount(a));
      if (sortMode === "occupancy") return (activeCount(b) / Math.max(1, b.capacity)) - (activeCount(a) / Math.max(1, a.capacity));
      return a.platform.name.localeCompare(b.platform.name, "zh-CN") || a.slotNumber - b.slotNumber;
    });
  }, [slots, deferredQuery, statusFilter, sortMode]);
